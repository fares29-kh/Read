...
try {
    ...
    $conn->exec("insert into `admin`(`nom`,`prenom`,`date_naiss`,`sexe`,`statut`,`id_admin`) value('$prenom','$nom','$date','$sexe','$etat','$last_id');");
   
    $conn->commit();
    header('Location: inscription_admin.php?erreur=1');
    
} catch (PDOException $e) {
    
    $conn->rollback();
    header('Location: inscription_admin.php?erreur=2');
    $e->getMessage();
}
